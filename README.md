...
Simulate uses an extra-compact variant of that heading and keeps `Back to notebook` plus the report gate in a compact row at the bottom of the right-hand comparison column. It no longer reserves a full-width footer, leaving the left column's lower area available for the benthic animation. Report retains the larger final-submission footer.
